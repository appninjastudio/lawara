...
export default function FoyUpdatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const inp = (label: string, key: string, type = 'text', ro = false) => (
    ...
    <input type={type} value={(dosyaForm as Record<string, unknown>)[key] as string || ''} readOnly={ro} onChange={e => setDosyaForm(p => ({ ...p, [key]: e.target.value }))} className={clsx('w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-icra-mid', ro && 'bg-slate-50')} /></div>
  );
  const sel = (label: string, key: string, opts: {value:string;label:string}[]) => (
    <div><label className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">{label}</label>
    <select value={(dosyaForm as Record<string, unknown>)[key] as string || ''} onChange={e => setDosyaForm(p => ({ ...p, [key]: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-icra-mid">{opts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
  );
        ...
        <div className="p-6 space-y-5">
          ...
          {activeTab === 'dosya' && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-end pb-1"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={dosyaForm.mtsTakibi} onChange={e => setDosyaForm(p => ({ ...p, mtsTakibi: e.target.checked }))} className="w-4 h-4 rounded border-slate-300 text-icra-mid" /><span className="text-sm text-slate-700">MTS Takibi</span></label></div>
              </div>
            ...
            <div key={t.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
              <div className="flex items-center gap-3 mb-4">
                <div className={clsx('w-10 h-10 rounded-full flex items-center justify-center', t.tarafRolu === 'alacakli' ? 'bg-emerald-100' : t.tarafRolu === 'borclu' ? 'bg-red-100' : 'bg-amber-100')}>
                  {t.kisiTuru === 'tuzel' ? <Building2 className="w-5 h-5 text-icra-mid" /> : <User className={clsx('w-5 h-5', t.tarafRolu === 'borclu' ? 'text-red-600' : 'text-emerald-600')} />}
                </div>
              ...
              {t.mernisAdresi && <div className="mt-2 p-2 bg-icra-light/10 rounded-lg"><p className="text-[10px] text-icra-mid">MERNİS ({fd(t.mernisTarihi)})</p><p className="text-sm">{t.mernisAdresi}</p></div>}
          ...
          {activeTab === 'alacak' && hesaplanan && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between py-3 bg-icra-light/10 rounded-lg px-3 mt-3"><span className="font-bold text-icra-dark">TOPLAM ALACAK</span><span className="text-lg font-bold text-icra-dark">{fc(hesaplanan.toplamAlacak)}</span></div>
          ...
          {activeTab === 'islemler' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => { setMForm({ tarih: new Date().toISOString().split('T')[0], tip: 'not_eklendi', aciklama: '' }); setShowModal('islem'); }} className="px-4 py-2 bg-icra-mid text-white rounded-lg text-xs font-medium hover:bg-icra-dark flex items-center gap-1.5 cursor-pointer"><Plus className="w-3.5 h-3.5" /> İşlem Ekle</button>
              </div>
              <div className="relative">
                ...
                {localIslemler.sort((a, b) => b.islemTarihi.localeCompare(a.islemTarihi)).map(is => (
                  <div key={is.id} className="relative pl-12 pb-4">
                    <div className={clsx('absolute left-3 w-5 h-5 rounded-full border-2 border-white shadow-sm', is.islemTipi.includes('tahsilat') ? 'bg-emerald-500' : is.islemTipi.includes('haciz') || is.islemTipi.includes('tapu') || is.islemTipi.includes('arac') ? 'bg-orange-500' : is.islemTipi.includes('tebligat') ? 'bg-icra-mid' : 'bg-slate-400')}><div className="w-full h-full flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-white" /></div></div>
                  ...
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedHaciz(expandedHaciz === h.id ? null : h.id)}>
                    <div className="flex items-center gap-3">
                      <div className={clsx('w-10 h-10 rounded-full flex items-center justify-center', h.hacizDurumu === 'olumlu_cevap' || h.hacizDurumu === 'uygulanidi' ? 'bg-emerald-100' : h.hacizDurumu === 'olumsuz_cevap' ? 'bg-red-100' : 'bg-amber-100')}>
                        {h.hacizTuru === 'banka' ? <Building2 className="w-5 h-5 text-icra-mid" /> : h.hacizTuru === 'arac' ? <Car className="w-5 h-5 text-purple-600" /> : h.hacizTuru === 'tasinmaz' ? <Landmark className="w-5 h-5 text-orange-600" /> : h.hacizTuru === 'maas' ? <Banknote className="w-5 h-5 text-cyan-600" /> : <Package className="w-5 h-5 text-slate-600" />}
                      </div>
          ...
          {activeTab === 'belgeler' && (() => {
            ...
            <div className="space-y-4">
              {/* Summary Bar */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-3 flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-icra-light/15 flex items-center justify-center"><FolderOpen className="w-4 h-4 text-icra-mid" /></div><div><p className="text-[10px] text-slate-500 uppercase">Toplam Belge</p><p className="text-lg font-bold text-slate-900">{localBelgeler.length}</p></div></div>
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-3 flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-icra-light/15 flex items-center justify-center"><ExternalLink className="w-4 h-4 text-icra-dark" /></div><div><p className="text-[10px] text-slate-500 uppercase">UYAP Belge</p><p className="text-lg font-bold text-icra-dark">{uyapCount}</p></div></div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <button onClick={() => { setMForm({ belgeAdi: '', dosyaAdi: '', kategori: 'diger', kaynak: 'manuel', aciklama: '', onemli: 'false', etiketler: '' }); setShowModal('belge'); }} className="px-4 py-2 bg-icra-mid text-white rounded-lg text-xs font-medium hover:bg-icra-dark flex items-center gap-1.5 cursor-pointer"><Upload className="w-3.5 h-3.5" /> Belge Ekle</button>
                </div>
              </div>
              {/* Belge Listesi */}
              {filtered.length === 0 ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              ...
                              <button className="p-1.5 rounded-lg text-icra-mid hover:bg-icra-light/10 cursor-pointer" title="İndir"><Download className="w-4 h-4" /></button>
                          ...
                          {b.etiketler.length > 0 && <div className="flex flex-wrap gap-1 mt-1.5">{b.etiketler.map((e, i) => <span key={i} className="px-1.5 py-0.5 bg-icra-light/10 text-icra-mid rounded text-[10px]">#{e}</span>)}</div>}
                        </div>
                      </div>
                    </div>
                    );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowModal(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <div className="p-5 border-b flex items-center justify-between">
              <h3 className={clsx('font-semibold flex items-center gap-2', showModal === 'gorusme' ? 'text-emerald-700' : showModal === 'ziyaret' ? 'text-amber-700' : showModal === 'tahsilat' ? 'text-emerald-700' : showModal === 'belge' ? 'text-icra-dark' : 'text-icra-dark')}>
            ...
            <div className="p-5 space-y-3">
              <div><label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Tarih</label><input type="date" value={mForm.tarih || ''} onChange={e => setMForm(p => ({ ...p, tarih: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-icra-mid" /></div>
              ...
              {showModal === 'belge' && <>
                <div><label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Belge Adı *</label><input type="text" value={mForm.belgeAdi || ''} onChange={e => setMForm(p => ({ ...p, belgeAdi: e.target.value }))} placeholder="ör. Ödeme Emri, Vekaletname..." className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-icra-mid" /></div>
                <div><label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Dosya Adı</label><input type="text" value={mForm.dosyaAdi || ''} onChange={e => setMForm(p => ({ ...p, dosyaAdi: e.target.value }))} placeholder="ör. odeme_emri.pdf" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-icra-mid" /></div>
                ...
                <div><label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Etiketler (virgülle ayırın)</label><input type="text" value={mForm.etiketler || ''} onChange={e => setMForm(p => ({ ...p, etiketler: e.target.value }))} placeholder="ör. haciz, banka, ödeme emri" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-icra-mid" /></div>
              ...
              {showModal !== 'belge' && <div><label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Açıklama</label><textarea rows={3} value={mForm.aciklama || ''} onChange={e => setMForm(p => ({ ...p, aciklama: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-icra-mid" /></div>}
              {showModal === 'belge' && <div><label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Açıklama (opsiyonel)</label><textarea rows={2} value={mForm.aciklama || ''} onChange={e => setMForm(p => ({ ...p, aciklama: e.target.value }))} placeholder="Belge hakkında kısa not..." className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-icra-mid" /></div>}
            </div>
            <div className="p-5 border-t flex gap-3 justify-end">
              ...
              <button onClick={handleModalSubmit} disabled={modalSaving} className={clsx('px-4 py-2 text-white rounded-xl text-sm font-medium disabled:opacity-50 flex items-center gap-2', showModal === 'gorusme' || showModal === 'tahsilat' ? 'bg-emerald-600 hover:bg-emerald-700' : showModal === 'ziyaret' ? 'bg-amber-600 hover:bg-amber-700' : 'bg-icra-mid hover:bg-icra-dark')}>
